...
function handleIntersection(entries, observer) {
  entries.forEach(entry => {
    if (entry.isIntersecting && entry.intersectionRatio >= 0.7) {
      const leftPanelAnchor = document.querySelector(`.js-api-toc [href="#${entry.target.getAttribute('aria-labelledby')}"]`);
      if (leftPanelAnchor) {
        const selectedItem = document.querySelector('.api-toc__item--selected');
        if (selectedItem) {
          selectedItem.classList.remove('api-toc__item--selected');
        }
        leftPanelAnchor.parentElement.classList.add('api-toc__item--selected');
      }
    }
  });
}
